...
def make_rotated_img_list(rotationInfo, img_list):
    ...
    if 90 in rotationInfo:
        for img_info in img_list:
            result_img_list.append(
                (img_info[0], cv2.rotate(img_info[1], cv2.ROTATE_90_COUNTERCLOCKWISE))
            )

    if 180 in rotationInfo:
        for img_info in img_list:
            result_img_list.append(
                (img_info[0], cv2.rotate(img_info[1], cv2.ROTATE_180))
            )

    if 270 in rotationInfo:
        for img_info in img_list:
            result_img_list.append(
                (img_info[0], cv2.rotate(img_info[1], cv2.ROTATE_90_CLOCKWISE))
            )
